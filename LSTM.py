...
# Create the input sequences and their corresponding labels for the LSTM model
...
# Plot the results
train = stock_data[:train_size+window_size]
test = stock_data[train_size+window_size:]
test['Predictions'] = predictions

plt.figure(figsize=(16, 8))
plt.title(f'Model Prediction Results - {ticker} Stock')
plt.xlabel('Date')
plt.ylabel('Close Price USD($)')
plt.plot(train.index, train['Close'], color='red', label='Training Data')
plt.plot(test.index, test['Close'], color='blue', label='Actual Data')
plt.plot(test.index, test['Predictions'], color='green', label='Predictions')
plt.savefig(f'{ticker}.png')
plt.legend()

# Print the prices of the predictions
for date, pred_price in zip(test.index, test['Predictions']):
    print(f"Price on {date.date()}: {pred_price}")

plt.show()

# Get the last 60 days of data
last_60_days = stock_data['Close'].values[-window_size:].reshape(-1, 1)
